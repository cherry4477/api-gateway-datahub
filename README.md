...
####b 用户在datahub上创建item，此时单点登录到api gateway上。此时页面向api gateway传递repository名称,用户名、token(http://http://plat.dataex.app-test.dataos.io/dataex-plat/ftl/dataex/api/creator/api_creator?reponame=xx&username=xx&token=xx)，并在本地查询token的真实性，若本地没有则到datahub验证token的合法性，若存在则信任，同时存储一份到本地。
